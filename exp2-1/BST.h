...
private:
  ...
  void printSequenceHelp(BSTNode<Key, E>*, int) const;
...
public:
  ...
  void printSequence()  // Print the contents of the BST as a sequence
  {
    if(root==NULL)
...
// Print out a BST as a sequence
template <typename Key, typename E>
void BST<Key, E>::
printSequenceHelp(BSTNode<Key, E>* root, int level) const {
  if(root == NULL)
  {
    cout << '/' << " ";
    return;
  ...
  {
    cout << root->element() << " ";
    printSequenceHelp(root->left(),level+1);
    printSequenceHelp(root->right(),level+1);
  }
}
